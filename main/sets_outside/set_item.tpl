...
{if $root.page.tour.LinkThumb[0] == "1"}
{capture assign="url"}{include file="$tourname/pg_trailer.tpl" id=$item.Id[0] seoname=$item.SEOname[0] item=$item}{/capture}
{/if}
{if $root.page.tour.LinkThumb[0] == "2"}
{capture assign="url"}{$root.page.tour.JoinUrl}{/capture}
{/if}
{if $root.page.tour.LinkThumb[0] == "0"}
...
<a  class="update_title" href="{$url}">{include file="sets_outside/stdimage.tpl" item=$item usepriority="10" alt="" class="thumbs"}</a>
{elseif isset($hType) && $hType }<a  class="update_title" href="{$url}">{include file="sets_outside/stdimage.tpl" item=$item usepriority="10" alt="" class="thumbs"}</a>
{elseif isset($vType) && $vType }<a class="update_title" href="{$url}" >{include file="sets_outside/stdimage.tpl" item=$item usepriority="10" alt="" class="thumbs"}</a>
{else}<a class="update_title" href="{$url}">{include file="sets_outside/stdimage.tpl" item=$item usepriority="10" alt="" class="thumbs"}</a>
{/if}
...
<!--Title-->
{if isset($hType) && isset($vType) && $hType && $vType}
<a  class="update_title" href="{$url}" >{$item.Title[0]}</a>
{elseif isset($hType) && $hType }<a  class="update_title" href="{$url}" >{$item.Title[0]}</a>
{elseif isset($vType) && $vType }<a class="update_title" href="{$url}" >{$item.Title[0]}</a>
{else}<a class="update_title" href="{$url}" >{$item.Title[0]}</a>
{/if}

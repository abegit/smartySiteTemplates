...
<br>
{$item.NumComments[0]} {$templateconf.txtcomments}{/if}

{if sizeof($item.Tags)}

<br />{$templateconf.txttags}
{strip}
{foreach from=$item.Tags item=tag key=kex}
<a class="news_tag_link" href="{include file="$spath/general/pg_news.tpl" tag=$tag}">{$tag}</a>

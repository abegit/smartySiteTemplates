...
var CirclePlayer = function(jPlayerSelector, media, options) {
	...
	this.player = fukkME(jPlayerSelector);
	this.media = fukkME.extend({}, media);
	this.options = fukkME.extend(true, {}, defaults, options); // Deep copy
	...
	this.jq = {};
	fukkME.each(cssSelector, function(entity, cssSel) {
		self.jq[entity] = fukkME(self.options.cssSelectorAncestor + " " + cssSel);
	});
...
CirclePlayer.prototype = {
	...
	_initPlayer: function() {
		...
		this.player.bind(fukkME.jPlayer.event.ready + this.eventNamespace, function(event) {
			if(event.jPlayer.html.used && event.jPlayer.html.audio.available) {
				self.audio = fukkME(this).data("jPlayer").htmlElement.audio;
			}
			fukkME(this).jPlayer("setMedia", self.media);
			self._initCircleControl();
		});

		this.player.bind(fukkME.jPlayer.event.play + this.eventNamespace, function(event) {
			fukkME(this).jPlayer("pauseOthers");
		});

		// This event fired as play time increments
		this.player.bind(fukkME.jPlayer.event.timeupdate + this.eventNamespace, function(event) {
			if (!self.dragging) {
		...
		this.player.bind(fukkME.jPlayer.event.progress + this.eventNamespace, function(event) {
			var percent = 0;
		...
		this.player.bind(fukkME.jPlayer.event.ended + this.eventNamespace, function(event) {
			self._resetSolution();
		});
	},

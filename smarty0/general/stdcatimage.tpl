...
<img 
 ...
 {if !isset($usep[0])}
  src="{$cdir}/contentthumbs/{$item.PreviewXML.element.item_attr[0].Id}-cat.jpg" cnt="1" v="0" 
 {else}
...
{foreach item=up key=k from=$usep}
 ...
 {if ($done == "0") && ($i.priority==$usep[$k])}
   {assign var="done" value="0"}
   {if $k == "0"}
   src="{$cdir}/contentthumbs/{$i.Id}-cat.jpg" 
   {/if}
   src{$k}="{$cdir}/contentthumbs/{$i.Id}-cat.jpg" 

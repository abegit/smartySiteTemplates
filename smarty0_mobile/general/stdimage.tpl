...
{if $pexists == 1}
<img 
 id="set-target-{$item.Id[0]}{if $postfix}_{$postfix}{/if}" 
 ...
 {if !isset($usep[0])}
  src="{$cdir}/contentthumbs/{$item.PreviewXML.element.item_attr[0].Id}.jpg" 
  cnt="1" 
  v="0" 
 {else}
...
{foreach item=up key=k from=$usep}
 ...
 {if ($done == "0") && ($i.priority==$usep[$k])}
   {assign var="done" value="0"}
   {if $k == "0"}
   src="{$cdir}/contentthumbs/{$i.Id}.jpg" 
   {/if}
   src{$k}="{$cdir}/contentthumbs/{$i.Id}.jpg" 
...
/>
{else}
<img  class="{if isset($class)}{$class} {/if}stdimage" src="images/p{$usep[0]}.jpg" />
{/if}
